    ...
    for r in score_by_results:
        ...
        while flag:
            ...
            if score_by_results[r][answer_match] < 0.45:
                ...
                score_by_results[r][answer_match]
                > score_by_answers[answer_match][max_result[0]]
            ):
...
def score_unstructured(year, answers, info_type):
    ...
    c_score = calc_score(
        [translation[res] if res in translation else res for res in results],
        answers[info_type],
    )
    ...
    for y in years:
        ...
        for g in grading:
            if g in ["hosts", "awards"]:
                (
                    scores[y][g]["spelling"],
                    scores[y][g]["completeness"],
                ) = score_unstructured(y, answers, g)
            else:
                (
                    scores[y][g]["spelling"],
                    scores[y][g]["completeness"],
                ) = score_structured(y, answers, g)
